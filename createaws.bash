...
if [[ ( $1 != "" && $2 != "") ]]; then
    ...
    KEY="$1"
    NAME="$2"
    INSTANCE="$(aws ec2 run-instances --image-id $AMI --count 1 --instance-type t2.small --key-name $KEY --security-groups jenkins | grep InstanceId)"
    ...
    #ssh-keyscan -H ${ID1} >> ~/.ssh/known_hosts
